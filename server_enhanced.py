...
# -*- coding: utf-8 -*-
"""
粤语发音服务器 - 支持Web Speech API和静态部署的版本
"""
...
class CantoneseTTS:
    def __init__(self):
        ...
        self.use_web_speech = False  # 标记是否使用Web Speech API模式
        self.check_support()
    ...
    def check_support(self):
        ...
        try:
            ...
            if 'Darwin' not in result.stdout:
                print("⚠️ 非macOS系统，启用Web Speech API模式")
            ...
            if 'Sinji' not in result.stdout:
                print("⚠️ 粤语语音包Sinji未安装，启用Web Speech API模式")
                self.use_web_speech = True
                return False
    ...
    def get_status(self):
        ...
        return {
            ...
            'supported': not self.use_web_speech,
            'use_web_speech': self.use_web_speech,
            'platform': 'server' if not self.use_web_speech else 'web_speech'
        }
...
def health():
    ...
    return jsonify({
        ...
        'supported': not tts.use_web_speech,
        'use_web_speech': tts.use_web_speech,
        'platform': 'server' if not tts.use_web_speech else 'web_speech'
    })
    ...
    <body>
        <div class="container">
            ...
            <div class="deploy-options">
                <h2><i class="fas fa-rocket"></i> 免费部署选项</h2>
                
                <div class="deploy-option">
                    <h3>选项一：GitHub Pages（推荐）</h3>
                    <p>完全免费，适合静态网站。发音功能使用浏览器原生的Web Speech API。</p>
                    <p><strong>优点：</strong> 完全免费，无需服务器，部署简单</p>
                    <p><strong>限制：</strong> 发音功能取决于浏览器支持，粤语支持可能有限</p>
                </div>
                
                <div class="deploy-option">
                    <h3>选项二：Vercel / Netlify</h3>
                    <p>支持静态文件和Serverless函数，有免费额度。</p>
                    <p><strong>优点：</strong> 支持后端API，发音功能更完整</p>
                    <p><strong>限制：</strong> Serverless函数有调用限制，需要配置API</p>
                </div>
            </div>
            ...
            <div class="info-section">
                ...
                <div class="info-item">
                    <strong>发音功能：</strong> 点击汉字卡片可以听到粤语发音（需要macOS系统或支持粤语的浏览器）
                </div>
                ...
                <div class="info-item">
                    <strong>文档：</strong> 
                    查看 <a href="DEPLOYMENT.md" style="color: #667eea;">DEPLOYMENT.md</a> 了解更多详情
                </div>
            </div>
        </div>
    </body>
...
if __name__ == '__main__':
    ...
    if tts.use_web_speech:
        print("⚠️  当前系统不支持macOS say命令")
        print("✅ 已启用Web Speech API兼容模式")
    else:
        print("✅ 系统支持粤语发音")
